...
export class DynQuestionComponent implements OnInit {
    ...
    constructor(private http: HttpClient, private ngxModal: NgxSmartModalService) {}
    ...
    searchOptions() {
        ...
        if (value !== '') {
            this.fetchOptions(this.question['endpoint'], this.question['param'], value).subscribe(resp => {
     ...
     */
    getOptions() {
        this.fetchOptions(this.question['endpoint'], '', '').subscribe(resp => {
            this.question['options'] = [];
            this.question['options'] = resp['result'].data.map(item => ({
                key: item[this.question['map'].key],
                value: item[this.question['map'].value]
