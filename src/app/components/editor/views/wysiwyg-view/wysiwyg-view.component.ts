...
export class WysiwygViewComponent implements OnInit, OnDestroy, AfterViewChecked {
    ...
    config() {
        ...
        this.subscribeCNM = this._editorService.getCurrentNodeModify().subscribe(currentNode => {
            const element = this.xedit.nativeElement.querySelector(
                '[' + XeditMapper.TAG_UUID + '="' + currentNode.getUuid() + '"]'
            );
    ...
    private parseContentToWysiwygEditor(content) {
        ...
        Object.keys(content).forEach(property => {
            ...
            renderContent += this.parseContentToWysiwygEditorWrapper(
                property,
                content[property].editable,
                Converters.json2html(data, true, true, false, this.enableHover)
            );
        });
    ...
    private parseContentToWysiwygEditorWrapper(property, editable, content) {
        const START_TAG = editable
            ? XeditMapper.TAG_EDITOR + ' ' + XeditMapper.TAG_UUID + '="' + property + '"'
    ...
    chooseTarget(origin) {
        ...
        if (len === 1 && origin.hasAttribute('xe_section')) {
            ...
            } else if (node.nodeName === 'P' && node.children.length === 1 && node.children[0].nodeName === 'IMG') {
    ...
    setSelection(currentNode) {
        ...
        if (!isNil(currentNode.getSchema())) {
            ...
            this.currentNode
                .getTarget()
                .setAttribute(XeditMapper.ATTR_WYSIWYG_SELECTED, this.currentNode.getTarget().nodeName);
    ...
    private updateContextMenuActions(node) {
        ...
        const clickFunc = (currentNode: any, afterNode: any, strTemplate: string, child = false) => {
            ...
            DOM.element(currentNode).insertNode(
                Converters.json2html(Converters.addWrapJson(nodeTemplate), true, true, false, this.enableHover),
                afterNode,
                true
            );
    ...
    private defaultActions(node) {
        ...
        if (
            (!this.existAction(node, 'paste') || this.getAction(node, 'paste') === true) &&
            !isNil(this.copyAction) &&
            !isNil(node)
        ) {
