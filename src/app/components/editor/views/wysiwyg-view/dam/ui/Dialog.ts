...
const ATTRS_BY_TYPE = {
    'image': {
        ...
        'longdesc': { type: 'textbox', name: 'longdesc', size: 40, label: 'Descripción' }
    },
    'link': {
        'title': { type: 'textbox', name: 'title', size: 40, label: 'Título' },
        'target': {
            type: 'listbox', name: 'target', label: 'Target', values:
                [{ text: 'Nueva ventana', value: '_blank' }, { text: 'Misma ventana', value: '_self' }]
        },
    }
};
...
const open = function (editor, http: HttpClient, type: string) {
    ...
    function openTree(evt, windowM, pathIds) {
        window['treeModal'].openModal('modal-1', type, pathIds).then(
            selectedId => {
    ...
    function showWManager(result, { editor }) {
        let name = result && result.name ? result.name : '<i>Elemento no seleccionado...</i>';
        let path = '<i>Elemento no seleccionado...</i>';
        if (result && result.path) {
            path = Object.values(result.path).join('/');
            path = `<span title="${path}">${path}<span/>`
        ...
        form.body.push({
            type: 'container', label: 'Enlace', layout: 'flow', direction: 'row', align: 'center', maxHeight: 50, height: 30,
            items: [
                { id: 'dam-nodeId', type: 'textbox', name: 'nodeId', label: 'textbox', readonly: true, value: currentId, required: true },
                { type: 'button', icon: 'browse', onclick: (e) => openTree(e, editor.windowManager, pathIds) }
            ]
        });
        form.body.push({ type: 'container', label: 'Nombre', id: 'dam-name', html: name });
        form.body.push({ type: 'container', label: 'Path', id: 'dam-path', html: path });
    ...
    if (currentId) {
        Api.getInfoNode(http, currentId, type, showWManager, showWManager, { editor: editor })
    } else {
