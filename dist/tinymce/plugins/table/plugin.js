...
(function(domGlobals) {
    ...
    var invalidTypeMessage = function(incorrect, type) {
        throw new Error(
            'All values need to be of type: ' + type + '. Keys (' + sort(incorrect).join(', ') + ') were not.'
        );
    };
    ...
    var MixedBag = function(required, optional) {
        ...
        return function(obj) {
            ...
            each(optional, function(opt) {
                r[opt] = constant(
                    Object.prototype.hasOwnProperty.call(obj, opt) ? Option.some(obj[opt]) : Option.none()
                );
            });
            return r;
        };
    ...
    var mergable = function(table, selections) {
        return SelectionTypes.cata(
            ...
            function(cells, _env) {
                ...
                return TableSelection.retrieveBox(
                    table,
    ...
    var extractSelected = function(cells) {
        return TableLookup.table(cells[0])
            .map(deep)
    ...
    var convert = function(cell, number, getter, setter) {
        var newSize = TableLookup.table(cell)
            .map(function(table) {
    ...
    var notStartRow = function(grid, rowIndex, colIndex, comparator) {
        return (
            GridRow.getCellElement(grid[rowIndex], colIndex) !== undefined &&
            (rowIndex > 0 &&
                comparator(
                    GridRow.getCellElement(grid[rowIndex - 1], colIndex),
    ...
    var insertRowsBefore = function(grid, details, comparator, genWrappers) {
        ...
        var newGrid = foldl(
            rows,
            function(newG, _row) {
                return ModificationOperations.insertRowAt(
                    newG,
                    targetIndex,
                    example,
                    comparator,
    ...
    var insertColumnBefore = function(grid, detail, comparator, genWrappers) {
        ...
        var newGrid = ModificationOperations.insertColumnAt(
            grid,
            targetIndex,
            example,
            comparator,
    ...
    var insertColumnsBefore = function(grid, details, comparator, genWrappers) {
        ...
        var newGrid = foldl(
            columns,
            function(newG, _row) {
                return ModificationOperations.insertColumnAt(
                    newG,
                    targetIndex,
                    example,
                    comparator,
    ...
    var splitCellIntoColumns$1 = function(grid, detail, comparator, genWrappers) {
        var newGrid = ModificationOperations.splitCellIntoColumns(
            grid,
            detail.row(),
            detail.column(),
            comparator,
    ...
    var splitCellIntoRows$1 = function(grid, detail, comparator, genWrappers) {
        var newGrid = ModificationOperations.splitCellIntoRows(
            grid,
            detail.row(),
            detail.column(),
            comparator,
    ...
    var eraseColumns = function(grid, details, _comparator, _genWrappers) {
        var columns = uniqueColumns(details);
        var newGrid = ModificationOperations.deleteColumnsAt(
            grid,
            columns[0].column(),
    ...
    var extractAdvancedStyles = function(dom, elm) {
        ...
        var borderColor = getRaw(Element.fromDom(elm), 'border-color')
            .map(rgbToHex)
            .getOr('');
        var bgColor = getRaw(Element.fromDom(elm), 'background-color')
            .map(rgbToHex)
            .getOr('');
        return {
    ...
    var extractDataFromTableElement = function(editor, elm, hasAdvTableTab) {
        var getBorder = function(dom, elm) {
            ...
            return (
                ...
                Styles$1.getTDTHOverallStyle(editor.dom, elm, 'border-width') ||
                Styles$1.getTDTHOverallStyle(editor.dom, elm, 'border')
            );
        };
        var dom = editor.dom;
        var data = __assign(
            {
                ...
                cellpadding:
                    dom.getAttrib(elm, 'cellpadding') || Styles$1.getTDTHOverallStyle(editor.dom, elm, 'padding'),
        ...
        if (!eq(start, finish)) {
            return CellSelection.identify(start, finish, isRoot).bind(function(cellSel) {
                ...
                if (boxes.length > 0) {
                    ...
                    return Option.some(
                        Response.create(Option.some(Util.makeSitus(start, 0, start, getEnd(start))), true)
                    );
                } else {
                    return Option.none();
                }
    ...
    var adjustTil = function(bridge, movement, original, caret, numRetries) {
        ...
        return bridge.situsFromPoint(caret.left(), movement.point(caret)).bind(function(guess) {
            return guess.start().fold(
                Option.none,
                function(element) {
                    return Rectangles.getEntireBox(bridge, element)
                            ...
                            return navigation.fold(
                                function() {
                                    return CellSelection.getEdges(
                                        ...
                                        var relative =
                                            SelectionKeys.isDown(keycode) || direction.isForward(keycode)
                                                ? Situ.after
                                                : Situ.before;
