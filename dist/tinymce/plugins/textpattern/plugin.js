...
(function(domGlobals) {
    ...
    var get$1 = function(patternsState) {
        ...
        var getPatterns = function() {
            return map(patternsState.get().inlinePatterns, denormalizePattern).concat(
    ...
    var findPattern = function(patterns, text) {
        return find(patterns, function(pattern) {
            ...
            if (
                isInlinePattern$1(pattern) &&
                pattern.end &&
                text.lastIndexOf(pattern.end) !== text.length - pattern.end.length
            ) {
                return false;
            }
            return true;
    ...
    var outcome = Adt.generate([{ aborted: [] }, { edge: ['element'] }, { success: ['info'] }]);
    var phase = Adt.generate([{ abort: [] }, { kontinue: [] }, { finish: ['info'] }]);
    ...
    var findPatternStart = function(dom, pattern, node, offset, block, requireGap) {
        ...
        return textBefore(node, offset, block).bind(function(spot) {
            var outcome = repeatLeft(
                dom,
                spot.element,
                spot.offset,
                nodeMatchesPattern(dom, block, pattern.start),
                block
            );
